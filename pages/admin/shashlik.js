...
const categoryOptions = ['shashlik'];

export default function AdminShashlik() {
  ...
  const addItem = async () => {
    ...
    const res = await fetch('/api/menu?menuType=shashlik', {
      method: 'POST',
    ...
    setShashlikData({
      ...shashlikData,
      [categoryKey]: [...(shashlikData[categoryKey] || []), item],
    });
    resetForm();
  };
  ...
  const deleteItem = async (id) => {
    ...
    const next = { ...shashlikData };
    for (const key of Object.keys(next)) {
      next[key] = (next[key] || []).filter((x) => x.id !== id);
    }
    setShashlikData(next);
  };
  ...
  // Helpers
  const allKeys = Object.keys(shashlikData);
    ...
    <div className="min-h-screen w-full bg-base text-white">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl md:text-3xl font-semibold text-white">Shashlik Menu</h1>
        ...
        {visibleKeys.length === 0 ? (
          ...
          visibleKeys.map((key) => {
            const items = (shashlikData[key] || []).filter((it) =>
              !query.trim() || 
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    // Revert on failure
                                    const reverted = { ...shashlikData };
                                    setShashlikData(reverted);
                                  }
                                }}
                                className={`w-8 h-4 rounded-full transition-colors ${
