...
const categoryOptions = ['hookah'];

export default function AdminHookah() {
  ...
  const addItem = async () => {
    ...
    const res = await fetch('/api/menu?menuType=hookah', {
      method: 'POST',
    ...
    setHookahData({
      ...hookahData,
      [categoryKey]: [...(hookahData[categoryKey] || []), item],
    });
    resetForm();
  };
  ...
  // Helpers
  const allKeys = Object.keys(hookahData);
    ...
    <div className="min-h-screen w-full bg-base text-white">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl md:text-3xl font-semibold text-white">Hookah Menu</h1>
        ...
        {visibleKeys.length === 0 ? (
          ...
          visibleKeys.map((key) => {
            const items = (hookahData[key] || []).filter((it) =>
              !query.trim() || 
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    // Revert on failure
                                    const reverted = { ...hookahData };
                                    setHookahData(reverted);
                                  }
                                }}
                                className={`w-8 h-4 rounded-full transition-colors ${
