...
const categoryOptions = ['cold_appetizer', 'salad', 'first_course', 'second_course'];

export default function AdminUzbek() {
  ...
  const addItem = async () => {
    ...
    setUzbekData({
      ...uzbekData,
      [categoryKey]: [...(uzbekData[categoryKey] || []), item],
    });
    resetForm();
  };
  ...
  // Helpers
  const allKeys = Object.keys(uzbekData);
    ...
    <div className="min-h-screen w-full bg-base text-white">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl md:text-3xl font-semibold text-white">Uzbek Menu</h1>
        ...
        {visibleKeys.length === 0 ? (
          ...
          visibleKeys.map((key) => {
            const items = (uzbekData[key] || []).filter((it) =>
              !query.trim() || 
                        ...
                        <div className="space-y-2">
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                            <div className="md:col-span-3">
                              ...
                              <select className="w-full p-2 rounded-md bg-black/20 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#e0d3a3] text-white" value={editCategoryKey} onChange={(e) => setEditCategoryKey(e.target.value)}>
                                {categoryOptions.map((k) => (
                                  <option key={k} value={k}>{k}</option>
                                ))}
                              </select>
                            ...
                            <div className="md:col-span-6">
                              ...
                              />
                              {editImagePreview && (
                                <img src={editImagePreview} alt="Preview" className="w-16 h-16 object-cover rounded mt-2" />
                              )}
                            </div>
                            <div className="md:col-span-3">
                              <div className="flex items-center gap-2 mb-1">
