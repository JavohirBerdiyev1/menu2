...
const categoryOptions = ['garnish'];

export default function AdminGarnish() {
  ...
  const addItem = async () => {
    ...
    const res = await fetch('/api/menu?menuType=garnish', {
      method: 'POST',
    ...
    setGarnishData({
      ...garnishData,
      [categoryKey]: [...(garnishData[categoryKey] || []), item],
    });
    resetForm();
  };
  ...
  // Helpers
  const allKeys = Object.keys(garnishData);
    ...
    <div className="min-h-screen w-full bg-base text-white">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl md:text-3xl font-semibold text-white">Garnish Menu</h1>
        ...
        {visibleKeys.length === 0 ? (
          ...
          visibleKeys.map((key) => {
            const items = (garnishData[key] || []).filter((it) =>
              !query.trim() || 
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    // Revert on failure
                                    const reverted = { ...garnishData };
                                    setGarnishData(reverted);
                                  }
                                }}
                                className={`w-8 h-4 rounded-full transition-colors ${
