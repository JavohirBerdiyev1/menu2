...
export default function AdminDynamicMenu() {
  ...
  const handleImageChange = (e, isEdit = false) => {
    ...
    reader.onload = (ev) => {
      if (isEdit) {
        setEditImage(file)
        setEditImagePreview(String(ev.target?.result || ''))
      } else {
        setImage(file)
  ...
  const addItem = async () => {
    ...
    const res = await fetch(`/api/menu?menuType=${menuType}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ categoryKey, item }),
    })
    if (!res.ok) return
    setData({ ...data, [categoryKey]: [...(data[categoryKey] || []), item] })
    resetForm()
  }
  ...
  const startEdit = (it, currentCategory) => {
    setEditingId(it.id)
    setEditCategoryKey(currentCategory)
  ...
  const saveEdit = async () => {
    ...
    const res = await fetch(`/api/menu?menuType=${menuType}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ id: editingId, updates, categoryKey: editCategoryKey }),
    })
    if (!res.ok) return
  ...
  const orderedKeys = useMemo(() => allKeys.sort(), [allKeys])
    ...
    <div className="min-h-screen w-full bg-base text-white">
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl md:text-3xl font-semibold text-white">{menuType ? keyToLabel(String(menuType)) : 'Menu'} Admin</h1>
        ...
        <div className="bg-white/5 border border-white/10 rounded-xl shadow-lg backdrop-blur p-4 md:p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4">
            <div className="md:col-span-3">
              ...
              <input className="w-full p-2 rounded-md bg-black/20 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#e0d3a3] text-white" placeholder="e.g. fast_food, burgers" value={categoryKey} onChange={(e) => setCategoryKey(e.target.value)} />
            </div>
            <div className="md:col-span-3">
            ...
            <div className="md:col-span-3">
              ...
              <input className="w-full p-2 rounded-md bg-black/20 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#e0d3a3] text-white" placeholder="Русское название" value={name.ru} onChange={(e) => setName({ ...name, ru: e.target.value })} />
            </div>
            <div className="md:col-span-3">
            ...
            <div className="md:col-span-6">
              <label className="block text-sm text-gray-300 mb-1">Image (Max 1MB)</label>
              <input type="file" accept="image/*" className="w-full p-2 rounded-md bg-black/20 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#e0d3a3] text-white" onChange={(e) => handleImageChange(e, false)} />
              {imagePreview && <img src={imagePreview} alt="Preview" className="w-16 h-16 object-cover rounded mt-2" />}
            </div>
            <div className="md:col-span-3">
        ...
        {visibleKeys.length === 0 ? (
          ...
          visibleKeys.map((key) => {
            const items = (data[key] || []).filter((it) => {
              const uz = String(it.name?.uz || '').toLowerCase()
              const ru = String(it.name?.ru || '').toLowerCase()
              const en = String(it.name?.en || '').toLowerCase()
              const q = query.toLowerCase()
              return !q || uz.includes(q) || ru.includes(q) || en.includes(q)
            })
            return (
                      ...
                      {editingId === it.id ? (
                        <div className="space-y-2">
                          <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                            <div className="md:col-span-3">
                              ...
                              <input className="w-full p-2 rounded-md bg-black/20 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#e0d3a3] text-white" value={editCategoryKey} onChange={(e) => setEditCategoryKey(e.target.value)} />
                            </div>
                            <div className="md:col-span-3">
                            ...
                            <div className="md:col-span-6">
                              <label className="block text-xs text-gray-300 mb-1">Image (Max 1MB)</label>
                              <input type="file" accept="image/*" className="w-full p-2 rounded-md bg-black/20 border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#e0d3a3] text-white" onChange={(e) => handleImageChange(e, true)} />
                              {editImagePreview && <img src={editImagePreview} alt="Preview" className="w-16 h-16 object-cover rounded mt-2" />}
                            </div>
                            <div className="md:col-span-3">
                              <div className="flex items-center gap-2 mb-1">
                                <label className="block text-xs text-gray-300">Show Description</label>
                                <button type="button" onClick={() => setEditShowDescription(!editShowDescription)} className={`w-6 h-3 rounded-full transition-colors ${editShowDescription ? 'bg-[#e0d3a3]' : 'bg-gray-600'}`}>
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex items-center gap-2 mr-2">
                              ...
                              <button type="button" onClick={() => setEditShow(!editShow)} className={`w-6 h-3 rounded-full transition-colors ${editShow ? 'bg-[#e0d3a3]' : 'bg-gray-600'}`}>
                                <div className={`w-2 h-2 bg-white rounded-full transition-transform ${editShow ? 'translate-x-3' : 'translate-x-0.5'}`} />
                              </button>
                            </div>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500 mt-1">Description: {it.showDescription ? 'Shown' : 'Hidden'} on page</div>
                          </div>
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  ...
                                  for (const k of Object.keys(optimistic)) {
                                    optimistic[k] = (optimistic[k] || []).map((x) => (x.id === it.id ? { ...x, show: nextShow } : x))
                                  }
                                  setData(optimistic)
                                  try {
                                    const res = await fetch(`/api/menu?menuType=${menuType}` , {
                                ...
                                className={`w-8 h-4 rounded-full transition-colors ${(it.show !== false) ? 'bg-[#e0d3a3]' : 'bg-gray-600'}`}
                              >
                                <div className={`w-3 h-3 bg-white rounded-full transition-transform ${(it.show !== false) ? 'translate-x-4' : 'translate-x-0.5'}`} />
                              </button>
                            </div>
